...
function showExport() {
    ...
    var uri = "data:text/plain;charset=utf-8," + content;
    $('export-link').writeAttribute('href', uri);
    $('export-link').writeAttribute('download', 'exported.mathics');
}

function showImport() {
    showPopup($('import'));

    document.getElementById('importWorksheet').addEventListener('change', function(e) {
        var files = e.target.files;

        if (files.length > 0) {
            var reader = new FileReader();
            reader.onload = function(e) {
                var worksheet = window.JSON.parse(e.target.result);
                setContent({
                    evalJSON: function() {
                        return worksheet;
                    }
        ...
        document.getElementById('importWorksheet').removeEventListener();
    });
}
